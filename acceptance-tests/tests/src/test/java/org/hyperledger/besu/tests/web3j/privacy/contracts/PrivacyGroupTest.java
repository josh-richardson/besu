...
public class PrivacyGroupTest extends AcceptanceTestBase {
  ...
  public void setUp() throws Exception {
    ...
    cluster.start(node);
    privacyGroup = node.execute(contractTransactions.createSmartContract(PrivacyGroup.class));
    web3j = Web3j.build(new HttpService(node.getHostName() + ":" + node.getJsonRpcSocketPort()));
  }
